...
OpentokAdapter.prototype.connect = function (callback) {
  var self = this
  this.session = opentok.initSession(this.apiKey, this.sessionId)
...
OpentokAdapter.prototype.publish = function (publishElement, callback) {
  if (!this.session) {
    this.connect()
  }
  var self = this
  this.publisher = opentok.initPublisher(publishElement, VIDEO_OPTIONS, function (err) {
    callback && callback(err)
    if (!err) {
